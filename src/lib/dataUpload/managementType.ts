...
export function resolveManagementType(
  ...
  const normalizedRepairType = normalizeRepairType(repairType);
  if (!normalizedRepairType) {
    return { managementType: null, warning: "수리유형 없음 - 관리구분 미분류" };
  }
  const fromMaster = masterMap.get(normalizedRepairType);
  if (fromMaster) return { managementType: fromMaster, warning: null };

  const fromDefault = DEFAULT_MANAGEMENT_TYPE_MAP[normalizedRepairType];
  if (fromDefault) return { managementType: fromDefault, warning: null };

  return {
    managementType: null,
    warning: `수리유형 미등록: "${normalizedRepairType}" - 관리구분을 자동으로 판정할 수 없어 미분류 처리됨`,
  };
}
